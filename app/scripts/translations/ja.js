...
X_HelpfulLinks_4                :  'MyHotelWalletから、またはそれに向けて移管中', /*Migrating to/from MyHotelWallet*/
...
ENS_Finalize_content            :  '終了処理を行うと、ドメイン名が落札者に移管されます。落札者には、第二位の入札価格との差額が返却されます。入札が一件だけの場合には、0.01ethとの差額が返却されます。落札者以外の入札も返却されます。', /*Finalizing this name assigns the ENS name to the winning bidder. The winner will be refunded the difference between their bid and the next-highest bid. If you are the only bidder, you will refunded all but 0.01 HBF. Any non-winners will also be refunded.*/
...
ENS_Helper_4                    :  'そのドメイン名に対して可能な最大の支払HBFを決めてください(自分の<u>Bid Amount</u>)。その口座に、入札に必要な額と0.01HBFのガス消費分が残っていることを確かめてください。', /*Decide the maximum amount of HBF you are willing to pay for the name (your <u>Bid Amount</u>). Ensure that account has enough to cover your bid + 0.01 HBF for gas.*/
...
ENS_Helper_16                   :  '落札者には、その入札額と二番入札との差額が返金されます。', /*The winner will be refunded the difference between their bid and the next-highest bid. If you are the only bidder, you will refunded all but 0.01 HBF.*/
...
ENS_Helper_21                   :  'イサーリアムネームサービスの改訂日の発表！', /*Announcing the Hotelbyte Name Service Relaunch Date!*/
...
EOS_06                          :  'Ether/トークンの送出ページに移動', /*Go to Send Hotelcoin & Tokens Page*/
...
/* Onboarding */
ONBOARD_welcome_title           :  'いらっしゃいませ！ MyHow.hotelbyte.foundation へようこそ！', /*Welcome to MyHotelWallet*/
...
ONBOARD_welcome_content__5      :  'MyHotelWalletはソースコード公開で、クライアント側にある無料で共用の利用者窓口です。', /*MyHotelWallet is a free, open-source, client-side interface.*/
...
ONBOARD_bank_title              :  'MyHotelWalletは金融機関にあらず　', /*MyHotelWallet is not a Bank*/
...
ONBOARD_interface_title         :  'MyHotelWalletは、共用の利用者窓口　', /*MyHotelWallet is an Interface*/
ONBOARD_interface_content__1    :  'MyHotelWalletを使って口座を作る時、一対の暗号のための二つの長い数字、すなわち秘密鍵と公開鍵（アドレス）とが作成されます。', /*When you create an account on MyHotelWallet you are generating a cryptographic set of numbers: your private key and your public key (address).*/
...
ONBOARD_interface_content__6    :  '公開鍵（アドレス）を相手に教えると、ＥＴＨやトークンを受け渡し出来る様になります。👍', /*If you send your *public key (address)* to someone, they can send you HBF or tokens. 👍*/
...
ONBOARD_bank_title__alt         :  'MyHotelWalletは金融機関にあらず　', /*MyHOW isn\'t a Bank*/
...
ONBOARD_blockchain_content__4   :  'ブロックチェーンのなかのブロックはMyHotelWallet, MetaMask, Exodus, Mist, Ghbc, Parityなど至る所で行われている個々の処理や取引で作られています。', /*The blocks in the blockchain are made up of all the individual transactions sent from MyHotelWallet, MetaMask, Exodus, Mist, Ghbc, Parity, and everywhere else.*/
ONBOARD_blockchain_content__5   :  'MyHotelWalletや[etherscan.io](https://etherscan.io)で残高や処理を見た時、見えるのは私たちのシステムの中ではなく、ブロックチェーンにあるデータです。', /*When you see your balance on MyHotelWallet or view your transactions on [explorer.hotelbyte.org](https://explorer.hotelbyte.org), you are seeing data on the blockchain, not in our personal systems.*/
ONBOARD_blockchain_content__6   :  'もう一度： **私たちは金融機関ではありません。**', /*Again: **we are not a bank**.*/
ONBOARD_interface_title__alt    :  'ＭＥＷは共用の利用者窓口　', /*MyHOW is an Interface*/
...
ONBOARD_blockchain_title__alt   :  '一体ブロックチェーンって何？ ', /*WTF is a Blockchain?*/
ONBOARD_point_title__alt        :  'それじゃＭＥＷは何ができるの？', /*What\'s the Point of MyHOW then?*/
ONBOARD_whymew_title            :  'MyHotelWalletにできないなら、どうすればいいの？', /*If MyHotelWallet can\'t do those things, what\'s the point?*/
...
ONBOARD_whymew_content__6       :  'もし、少し不安だけれどもＭＥＷを使いたければ、[ハードウェアワレットを入手(英語)](https://myetherwallet.github.io/knowledge-base/hardware-wallets/hardware-wallet-recommendations.html)！', /*f you are scared but want to use MyHOW, [get a hardware wallet](https://myetherwallet.github.io/knowledge-base/hardware-wallets/hardware-wallet-recommendations.html)! These keep your keys secure.*/
...
ONBOARD_secure_1_content__1     :  'オンライン詐欺師は、MyHotelWallet、EtherDelta、Paypalやあなたの金融機関のリンクが含まれた個人メッセージを送りつけてきますが、これは偽のサイトです。あなたの情報を盗み、そして資産を盗みます。', /*Phishers send you a message with a link to a website that looks just like MyHotelWallet, EtherDelta, Paypal, or your bank, but is not the real website. They steal your information and then steal your money.*/
...
ONBOARD_secure_1_content__3     :  'いつも、必ず、ＵＲＬ：「https://myhow.hotelbyte.foundation」を確認してください。', /*Always check the URL: `https://myhow.hotelbyte.foundation`.*/
ONBOARD_secure_1_content__4     :  'ＵＲＬバーに、 `MYHBFERWALLET LLC`と緑色で表示されていることを必ず確認してください。', /**/
...
ONBOARD_final_content__3        :  'ＭＥＷとＭｅｔａｍａｓｋの設定方法', /*How to Set up MyHOW + MetaMask*/
ONBOARD_final_content__4        :  'オフライン/手元でのＭＥＷの使い方', /*How to Run MyHOW Offline / Locally*/
...
/* Old */
ADD_DigitalBitbox_0a        : 'セキュアコネクション（SSL)で再度MyHotelWalletを開いてください。 ',
ADD_DigitalBitbox_0b        : 'MyHotelWalletを再度「Chrome」(https://www.google.com/chrome/browser/desktop/) あるいは [Opera](https://www.opera.com/)で開いてください。 ',
...
ADD_Ledger_0a               : 'セキュアコネクション（SSL)で再度MyHotelWalletを開いてください。 ',
ADD_Ledger_0b               : 'MyHotelWalletを再度「Chrome」(https://www.google.com/chrome/browser/desktop/) あるいは [Opera](https://www.opera.com/)で開いてください。 ',
...
ADD_Warning_1               : '任意のアカウントを監視する目的で、秘密鍵をアップロードせずにお財布タブに追加することできます。これによってお財布の操作や、HBFERの移動が可能になるわけではありません。 ',
...
ERROR_28                    : '(error_29) お財布を操作するためには**Keystore/JSONとパスワードか秘密鍵が必要** 保存してから、外部バックアップしてください！ここで保存しないとお財布が使用できなくなります。詳細はヘルプページを参照してください。(https://myhow.hotelbyte.foundation/#help) (You need this `Keystore File + Password` or the `Private Key` (next page) to access this wallet in the future. ) ',
...
GHBF_Balance                : '(geth-01) 残高不足 Insufficient balance. Your gas limit * gas price + amount to send exceeds your current balance. Send more HBF to your account or use the "Send Entire Balance" button. If you believe this is in error, try pressing generate again. Required (d+) and got: (d+). [Learn More.](https://myetherwallet.github.io/knowledge-base/transactions/transactions-not-showing-or-pending.html)',
GHBF_Cheap                  : '(geth-02) ガス価格が低すぎます Gas price too low for acceptance. Try raising the gas price to 21 GWEI via the dropdown in top-right.',
GHBF_GasLimit               : '(geth-03) ブロックガスリミットを越えています Exceeds block gas limit. Transaction cost exceeds current gas limit. Limit: (d+), got: (d+). Please lower the gas limit to 21000 (for sending) or 200000 (for sending tokens or contracts) and try again. [Learn More](https://myetherwallet.github.io/knowledge-base/gas/what-is-gas-ethereum.html)',
GHBF_InsufficientFunds      : '(geth-04) ガス*価格+数量に足りません Insufficient balance. Your gas limit * gas price + amount to send exceeds your current balance. Send more HBF to your account or use the "Send Entire Balance" button. If you believe this is in error, try pressing generate again. Required (d+) and got: (d+). [Learn More.](https://myetherwallet.github.io/knowledge-base/transactions/transactions-not-showing-or-pending.html)',
GHBF_IntrinsicGas           : '(geth-05) 基本のガス不足です Intrinsic gas too low. Try raising the gas price to 21 GWEI via the dropdown in top-right or the gas limit to 21000 (for sending) or 200000 (for sending tokens or contracts) and try again.',
GHBF_InvalidSender          : '(geth-06) 送出元が無効です Invalid sender.',
GHBF_NegativeValue          : '(geth-07) 負の値です Negative value.',
GHBF_Nonce                  : '(geth-08) Nonce が足りません This TX\'s [nonce](https://myetherwallet.github.io/knowledge-base/transactions/what-is-nonce.html) is too low. Try incrementing the nonce by pressing the Generate button again, or [replace the pending transaction](https://myetherwallet.github.io/knowledge-base/transactions/check-status-of-ethereum-transaction.html).',
GHBF_NonExistentAccount     : '(geth-09) アカウントが存在しない、あるいはその残高不足です Account does not exist or account balance too low',
...
GAS_LIMIT_Desc              : 'ガスリミットは、その処理のために送出するガスの総量です。「処理料金 = ガス価格 ＊ ガスリミット」で算出され、自分の処理をブロックに配置するための料金として支払われます。 この数字を増やしても、自分の処理が早くなることはありません。HBFの送出 = 「２１０００」、トークンの送出 = ~「２０００００」が目安になります。', /*Gas limit is the amount of gas to send with your TX. `TX fee` = gas price * gas limit & is paid to miners for including your TX in a block. Increasing this number will not get your TX mined faster. Sending HBF = `21000`. Sending Tokens = ~`200000`.*/
/* Gen Wallet Updates */
...
GEN_Help_12                 : 'MyHotelWalletの上でこれを使って自分のお財布をアンロックしてください（Mist, Ghbc, Parityや他のお財布クライアントも可）', /*Use it to unlock your wallet via MyHotelWallet (or Mist, Ghbc, Parity and other wallet clients.)*/
...
MYWAL_Content_3             : 'MyHotelWallet CXでこのお財布を使用するためには、秘密鍵あるいはJSONとパスワードを手動で追加する必要があります。 ',
...
NODE_Warning                : 'MyHow.hotelbyte.foundation 経由で接続するためには、HTTPSノードが必要です。[MyHotelWalletをレポジトリからダウンロードして、手元で走らせ、](https://github.com/kvhnuke/etherwallet/releases/latest)いずれかのノードに接続することもできます。 あるいは、無料のSSL証明書を[LetsEncrypt](https://letsencrypt.org/)から入手してください',
NODE_Name                   : 'ノード名',
NODE_Port                   : 'ノードポート',
...
SENDModal_Content_4         : 'ノート：最も起こりやすいエラーの原因は、送出のためのガス不足で、その場合には、HBFを追加する必要があります。ガスはHBFで支払われます。 ',
...
sidebar_donation            : 'MyHotelWalletは、プライバシーとセキュリティのための無料のオープンソースサービスです。　寄付が増えることによって、新機能やフィードバックの反映を行い、よりユーザーの皆様のご希望に沿った制作の時間を増やす事が可能になります。私たちは、たった二人で世界を変えようとしています。お手伝いいただけますか? ',
sidebar_donate              : '寄付する ',
...
SWAP_rates                  : "現在のレート ",
SWAP_init_1                 : "両替する ",
SWAP_init_2                 : " を ", // "I want to swap my X HBF for X BTC"
...
SWAP_progress_3             : "HBF受け取り完了 ", // HBF Received!
SWAP_progress_4             : "送出中 {{orderResult.output.currency}} ",
SWAP_progress_5             : "注文完了 ",
SWAP_order_CTA              : "お送りいただきたいのは ", // Please send 1 HBF...
SWAP_unlock                 : "このページから直接HBFあるいはトークンを送出するためには、お財布を解錠してください。",
...
TRANS_gas                   : 'ガスリミット： ',
TRANS_sendInfo              : '21000ガスを使用する標準トランザクションは、0.000441 HBFを消費します。迅速な処理を行うために、若干最小量よりも多めの0.000000021 HBFのガスを使用します。当サービスでは、トランザクション料金は徴収いたしません。 ',
...
tx_FoundOnChain_3           : '**何もエラーメッセージが返ってこなければ、あなたの処理は正しく送出されています。** HBFあるいはトークンは、送ろうとしたあて先の場所にあります。 もし、HBFやトークンが他のお財布や交換所のお財布に見つからず、処理を開始してから２４時間以上経っていたら、 [そのサービスに連絡](https://myetherwallet.github.io/knowledge-base/diving-deeper/ethereum-list-of-support-and-communities.html)してください。状況を確認してもらうために、自分の処理のリンクをうまく知らせてください。',
...
tx_notFound_4               : '右上の下展開メニューから、別のノードを選択してください。 (例： `HBF (Etherscan.io)` or `HBF (Infura.io)` or `HBF (MyHotelWallet)`) から選んで再度確認する。',
...
TXFEE_Desc                  : 'この処理料金は自分の処理をブロックに配置するためにマイナーに支払われます。「ガスリミット」＊「ガス価格」で算出されます。 [GWEI -> HBFの単位変換はこちらです。](https://myhow.hotelbyte.foundation/helpers.html)',　/*The TX Fee is paid to miners for including your TX in a block. It is the `gas limit` * `gas price`. [You can convert GWEI -> HBF here](https://myhow.hotelbyte.foundation/helpers.html)*/

VIEWWALLET_HidePrivKey      : '(hide)',
VIEWWALLET_ShowPrivKey      : '(show)',
VIEWWALLET_Subtitle         : '異なったバージョンの秘密鍵をダウンロードしたり、お財布紙情報を再印刷することができます。[import your account into Ghbc/Mist](http://ethereum.stackexchange.com/questions/465/how-to-import-a-plain-private-key-into-geth/)する時に必要です。残高をチェックするためには、[explorer.hotelbyte.org](https://explorer.hotelbyte.org)のようなブロックチェーンエクスプローラーサービスを使う事をおすすめします。 ',
...
x_TxHash                    : '処理ハッシュ',
x_Access                    : 'アクセス ',
x_AddessDesc                : '自分のアドレスは、アカウント番号や公開キーでも知ることができます。誰かとHBFやトークンの受け渡しをするために、これらを用います。きれいな色のついたアドレスアイコンを見つけてください。自分のペーパーウォレットや、アドレスを入力する際に、同じアイコンであることをしっかりと確認してください。 これは自分のアカウント番号と公開鍵になります。HBFを送信するために必要な情報です。アイコンは自分のアドレスを識別するものです。 ',
x_Address                   : '自分のアドレス ',
x_Cancel                    : '取り消す ',
x_CSV                       : 'CSV ファイル (未暗号化) ',
x_Download                  : 'ダウンロード ',
...
HELP_0_Desc_1               : 'MyHotelWallet gives you the ability to generate new wallets so you can store your Ether yourself, not on an exchange. This process happens entirely on your computer, not our servers. Therefore, when you generate a new wallet, **you are responsible for safely backing it up**. ',
...
HELP_2b_Title               : '2b) How do I safely / offline / cold storage with MyHotelWallet? ',
...
HELP_3_Desc_3               : 'Navigate to the MyHow.hotelbyte.foundation "View Wallet Info" page. ',
...
HELP_4_Title                : '4) How do I Send Hotelcoin from one wallet to another? ',
...
HELP_4CX_Title              : '4) How do I Send Hotelcoin using MyHotelWallet CX? ',
...
HELP_5_Title                : '5) How do I run MyHow.hotelbyte.foundation offline/locally? ',
HELP_5_Desc_1               : 'You can run MyHow.hotelbyte.foundation on your computer instead of from the GitHub servers. You can generate a wallet completely offline and send transactions from the "Offline Transaction" page. ',
HELP_5_Desc_7               : 'MyHow.hotelbyte.foundation is now running entirely on your computer. ',
HELP_5_Desc_8               : 'In case you are not familiar, you need to keep the entire folder in order to run the website, not just `index.html`. Don\'t touch or move anything around in the folder. If you are storing a backup of the MyHotelWallet repo for the future, we recommend just storing the ZIP so you can be sure the folder contents stay intact. ',
HELP_5_Desc_9               : 'As we are constantly updating MyHow.hotelbyte.foundation, we recommend you periodically update your saved version of the repo. ',
...
HELP_8_Desc_1               : 'MyHotelWallet is not a web wallet. You don\'t have a login and nothing ever gets saved to our servers. It is simply an interface that allows you interact with the blockchain. ',
HELP_8_Desc_2               : 'If MyHow.hotelbyte.foundation goes down, you would have to find another way (like geth or Hotelbyte Wallet / Mist) to do what we are doing. But you wouldn\'t have to "get" your Ether out of MyHotelWallet because it\'s not in MyHotelWallet. It\'s in whatever wallet your generated via our site. ',
HELP_8_Desc_3               : 'You can import your unencrypted private key and your Ghbc/Mist Format (encrypted) files directly into geth / Hotelbyte Wallet / Mist very easily now. See question #12 below. ',
HELP_8_Desc_4               : 'In addition, the likelihood of us taking MyHotelWallet down is slim to none. It costs us almost nothing to maintain as we aren\'t storing any information. If we do take the domain down, it still is, and always will be, publicly available at [https://github.com/kvhnuke/etherwallet](https://github.com/kvhnuke/etherwallet/tree/gh-pages). You can download the ZIP there and run it locally. ',
HELP_8CX_Title              : '8) What happens If MyHotelWallet cX disappears? ',
...
HELP_8CX_Desc_2             : 'That said, it is **very important** that you back up all your information for any new wallets generated with MyHotelWallet CX. That way if anything happens to MyHotelWallet CX or your computer, you still have all the information necessary to access your Ether. See the #2a for how to back up your wallets. ',
HELP_8CX_Desc_3             : 'If for some reason MyHotelWallet CX disappears from the Chrome Store, you can find the source on Github and load it manually. See #5 above. ',
...
HELP_12_Title               : '12) How do I import a wallet created with MyHotelWallet into geth / Hotelbyte Wallet / Mist? ',
HELP_12_Desc_1              : 'Using an Ghbc/Mist JSON file from MyHotelWallet v2+.... ',
...
HELP_12_Desc_6              : 'Find the "Download JSON file - Ghbc/Mist Format (encrypted)" section. Press the "Download" button below that. You now have your keystore file. ',
HELP_12_Desc_7              : 'Open the Hotelbyte Wallet application. ',
...
HELP_12_Desc_20             : 'This will prompt you to make a new password. This is the password you will use in geth / Hotelbyte Wallet / Mist whenever you send a transaction, so don\'t forget it. ',
HELP_12_Desc_21             : 'After successful import, delete `nothing_special_delete_me.txt` ',
HELP_12_Desc_22             : 'The next time you open the Hotelbyte Wallet application, your account will be listed under "Accounts". ',
...
HELP_12_Desc_26             : 'This will prompt you to make a new password. This is the password you will use in geth / Hotelbyte Wallet / Mist whenever you send a transaction, so don\'t forget it. ',
HELP_12_Desc_27             : 'After successful import, delete `nothing_special_delete_me.txt` ',
HELP_12_Desc_28             : 'The next time you open the Hotelbyte Wallet application, your account will be listed under "Accounts". ',
...
HELP_16_Desc_1              : 'You can use a blockchain explorer like [explorer.hotelbyte.org](https://explorer.hotelbyte.org). Paste your address into the search bar and it will pull up your address and transaction history. For example, here\'s what our [donation account](https://etherscan.io/address/0xDECAF9CD2367cdbb726E904cD6397eDFcAe6068D) looks like on etherscan.io ',
...
HELP_21_Desc_4              : 'Now, that said, YES if someone else has your private key then they can indeed Send Hotelcoin from your account. Just like if someone has your password to your email, they can read and send your email, or the password to your bank account, they could make transfers. You could download the Keystore version of your private key which is the private key that is encrypted with a password. This is like having a password that is also protected by another password. ',
...
HELP_Remind_Desc_1          : '**Hotelbyte, MyHow.hotelbyte.foundation & MyHotelWallet CX, and some of the underlying Javascript libraries we use are under active development.** While we have thoroughly tested & tens of thousands of wallets have been successfully created by people all over the globe, there is always the remote possibility that something unexpected happens that causes your HBF to be lost. Please do not invest more than you are willing to lose, and please be careful. If something were to happen, we are sorry, but **we are not responsible for the lost Ether**. ',
HELP_Remind_Desc_2          : 'MyHow.hotelbyte.foundation & MyHotelWallet CX are not "web wallets". You do not create an account or give us your Ether to hold onto. All data never leaves your computer/your browser. We make it easy for you to create, save, and access your information and interact with the blockchain. ',
...
HELP_SecCX_Title            : 'Security - MyHotelWallet CX ',
...
HELP_SecCX_Desc_6           : 'If we were to encrypt these items, you would need to enter a password each time you wanted to view your account balance or view the nicknames. If this concerns you, we recommend you use MyHow.hotelbyte.foundation instead of this Chrome Extension. ',
...
HELP_Sec_Desc_4             : 'Check the URL -- This site is being served through GitHub and you can see the source code here: [https://github.com/kvhnuke/etherwallet/tree/gh-pages](https://github.com/kvhnuke/etherwallet/tree/gh-pages) to [https://myhow.hotelbyte.foundation](https://myhow.hotelbyte.foundation). ',
