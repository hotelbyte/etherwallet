...
/* Onboarding */
ONBOARD_welcome_title           :  '歡迎來到 MyHow.hotelbyte.foundation',
...
ONBOARD_welcome_content__5      :  'MyHotelWallet 是一個免費、開源、運行於用戶端的介面。',
...
ONBOARD_bank_title              :  'MyHotelWallet 不是一間銀行',
...
ONBOARD_interface_title         :  'MyHotelWallet 是一個介面',
ONBOARD_interface_content__1    :  '當你在 MyHotelWallet 建立一個帳戶，你是在產生一串密碼數列：你的私鑰和你的公鑰 (地址)。',
...
tx_notFound_4               : '請用右上角的下拉式選單來選取其他 HBF 節點 (例如： `HBF (Etherscan.io)` 、 `HBF (Infura.io)` 或 `HBF (MyHotelWallet)`) 並再檢查一次。',
...
GEN_Help_12                 : '用它在 MyHotelWallet (或Mist、Geth、Parity和其他錢包) 來解鎖你的錢包 ',
...
sidebar_donation            : 'MyHotelWallet是注重你的隱私和安全的免費、開源服務。如果有更多捐贈，我們就能花更多時間開發新功能、聽取你的建議並且滿足你的需要。我們只是兩個想要改變世界的人。幫助我們？ ',
sidebar_donate              : '捐贈 ',
sidebar_thanks              : '謝謝你!!! ',
...
ADD_Ledger_0b               : '以 [Chrome](https://www.google.com/chrome/browser/desktop/) 或 [Opera](https://www.opera.com/) 瀏覽器重新開啟MyHotelWallet',
...
ADD_DigitalBitbox_0b        : '以 [Chrome](https://www.google.com/chrome/browser/desktop/) 或 [Opera](https://www.opera.com/) 瀏覽器重新開啟MyHotelWallet',
...
MYWAL_Content_3             : '如果你在未來還想在MyHotelWallet擴充功能上操作此錢包，你必須使用私鑰或JSON檔和密碼來重新新增它。 ',
...
TRANS_warning               : '"Only HBF" 或 "Only ETC" 功能是透過一個合約來傳送，並非單純的轉錢。有些服務可能沒辦法接受這兩種功能。 了解更多。Functions you are sending via a contract. Some services have issues accepting these transactions. Read more. ',
TRANS_advanced              : '進階功能：為交易新增Data',
TRANS_data                  : '交易的Data ',
TRANS_gas                   : 'Gas 總量 ',
TRANS_sendInfo              : '我們使用比平均手續費略高的gas價格（每單位0.000000021 HBF）確保交易能快速被確認。我們並不收取任何手續費。',
...
NODE_Warning                : '你的節點必須使用HTTPS安全連線的方式才能夠由 MyHow.hotelbyte.foundation 連接到它。 你可以下載 [MyHotelWallet repo](https://github.com/kvhnuke/etherwallet/releases/latest) 並建立起一個你的節點來連接至其他的節點。或著從 [LetsEncrypt](https://letsencrypt.org/) 獲取免費的SSL憑證',
NODE_Name                   : '節點名稱',
NODE_Port                   : '節點連線的端口號碼',
...
SWAP_rates                  : "目前的匯率 ",
SWAP_init_1                 : "我想要用我的 ",
SWAP_init_2                 : " 換 ", // "I want to swap my X HBF for X BTC"
...
/* View Wallet Details */
VIEWWALLET_Subtitle         : '這可以讓你下載不同版本的私鑰並且重新印製出你的紙錢包。你可能會需要這個功能來將你的帳戶[讀入Ghbc/Mist](http://ethereum.stackexchange.com/questions/465/how-to-import-a-plain-private-key-into-geth/)。 如果你想要檢查你的帳號餘額，我們建議你使用區塊鏈瀏覽網頁像是[explorer.hotelbyte.org](https://explorer.hotelbyte.org)。 ',
...
ERROR_17                    : '你帳戶中必須有至少 **0.01 HBF** 來支付gas的成本。 請增加一些 HBF 並重試。 ',
...
ERROR_28                    : '**以後你會需要密碼和Keystore檔案** (或私鑰)來存取你的錢包。請儲存並備份在額外的地方！如果沒有儲存你是沒有機會找回這個錢包的。 詳見[幫助頁面](https://myhow.hotelbyte.foundation/#help)來獲取更多資訊。 ',
...
SUCCESS_1                   : '有效的地址 ',
SUCCESS_2                   : '成功解密錢包 ',
SUCCESS_3                   : '你的交易已被發布到網路上。這並不表示該交易已被確認或完成。遇到區塊鏈網路流量極大時，你可能需要超過三個小時才能完成交易 1) 在下面確認你的交易狀態。 2) 如果該交易已經等待(pending)數小時或消失，使用 檢查交易狀態 頁面來替換它。 3) 使用 [HBF加油站](https://ethgasstation.info/) 來確認最佳的Gas價格。 4) 存下該筆交易的雜湊值，你可能需要它:   ', //'Your TX has been broadcast to the network. This does not mean it has been mined & sent. During times of extreme volume, it may take 3+ hours to send. 1) Check your TX below. 2) If it is pending for hours or disappears, use the Check TX Status Page to replace. 3) Use [HBF Gas Station](https://ethgasstation.info/) to see what gas price is optimal. 4) Save your TX Hash in case you need it later:    ', //
SUCCESS_4                   : '成功新增你的錢包 ',
SUCCESS_5                   : '已選擇檔案 ',
SUCCESS_6                   : '你已經成功連上 ',
SUCCESS_7                   : '訊息簽章驗證成功',
...
/* Geth Error Messages */
GHBF_InvalidSender          : '無效的傳送者 ',
GHBF_Nonce                  : 'Nonce值太低 ',
GHBF_Cheap                  : 'Gas價格太低以致不會被礦工接受 ',
GHBF_Balance                : '餘額不足 ',
GHBF_NonExistentAccount     : '帳戶不存在或是帳戶餘額不足 ',
GHBF_InsufficientFunds      : '提供的金額不足以支付gas成本的錢加上你要轉的錢 ',
GHBF_IntrinsicGas           : '提供的gas量不足 ',
GHBF_GasLimit               : 'gas量超過每單位區塊的gas上限 ',
GHBF_NegativeValue          : '負數的值 ',
